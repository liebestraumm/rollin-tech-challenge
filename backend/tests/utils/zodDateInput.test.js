...
const zodDateInput_1 = require("../../src/utils/zodDateInput");
const zod_1 = require("zod");
describe('zodDateInput Utility', () => {
    const schema = zod_1.z.object({
        testDate: (0, zodDateInput_1.zodDateInput)('Test Date'),
    });
    ...
    describe('invalid date handling', () => {
        it('should throw error for invalid date string', () => {
            expect(() => schema.parse({ testDate: 'invalid-date' })).toThrow(zod_1.ZodError);
            try {
                schema.parse({ testDate: 'invalid-date' });
            }
            catch (error) {
                if (error instanceof zod_1.ZodError) {
                    expect(error.issues[0].message).toBe('Test Date must be a valid date');
                }
            }
        });
        it('should throw error for a date that does not exist', () => {
            expect(() => schema.parse({ testDate: '2024/12/32' })).toThrow(zod_1.ZodError);
        });
        it('should throw error for non-date values', () => {
            expect(() => schema.parse({ testDate: 12345 })).toThrow(zod_1.ZodError);
        });
    });
});
